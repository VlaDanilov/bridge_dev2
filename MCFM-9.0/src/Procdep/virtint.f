...
!  this program. If not, see <http://www.gnu.org/licenses/>
     ...
     & msqx(0:2,-nf:nf,-nf:nf,-nf:nf,-nf:nf)
      ...
      integer itrial
      real(dp):: xmsqvar(6),alphas
      real(dp):: p(mxpart,4),pjet(mxpart,4),W,xmsq,
...
! adjust scales for scale variation
      if (doScalevar .and. currentPDF == 0 .and. bin) then
          if (itrial == 7) then
             call usescales(scale, facscale/2._dp)
          elseif (itrial == 6) then
              call usescales(scale, facscale*2._dp)
          elseif (itrial == 5) then
              call usescales(scale/2._dp, facscale)
          elseif (itrial == 4) then
              call usescales(scale*2._dp, facscale)
          elseif (itrial == 3) then
              call usescales(scale/2._dp, facscale/2._dp)
...
! Sanity check for tiny pt(top)/mt -- otherwise numerical Gram det. problems
