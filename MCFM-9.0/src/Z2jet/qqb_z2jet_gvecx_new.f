...
c--- extra term for gg -> qqb
     ...
     &      +abs(Q(nquark)*q1+R(nquark)*l1*prop)**2*ggZqqb_cs(icol,2,1)
            ...
            enddo
            msqv_cs(icol,j,k)=+3._dp*ppmsqvx(pp(j,k,-1,1))
     ...
     &       +abs(Q(j)*q1+R(j)*l1*prop)**2*qgZqg_cs(icol,2,1)
          enddo
          if (j <= 4) then
          ppmsqvx(pp(j,k,j,k))=msqv_cs(0,j,k)+msqv_cs(1,j,k)+msqv_cs(2,j,k)
          ppmsqvx(pp(j,k,k,j))=ppmsqvx(pp(j,k,j,k))
          endif
     ...
     &       +abs(Q(k)*q1+R(k)*l1*prop)**2*gqZqg_cs(icol,2,1)
          enddo
          if (k <= 4) then
          ppmsqvx(pp(j,k,k,j))=msqv_cs(0,j,k)+msqv_cs(1,j,k)+msqv_cs(2,j,k)
          ppmsqvx(pp(j,k,j,k))=ppmsqvx(pp(j,k,k,j))
          endif
