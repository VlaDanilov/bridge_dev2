...
! note that e^Y is given by Y = log(x1/x2)/two = yraptwo(3,4,p)
        eYonrho=sqrt(x1/x2)
      endif
...
! Note that the NNLO coefficients are computed only for keepSubL = F
      if (order == 2) then
...
! Fill array of power corrections
      do j=-nf,nf
        if (j == 0) cycle
